...
public static class Extensions
{
    ...
    public static void AlignAndPlaceItem(this ItemSpawner itemSpawner, Item item, System.Random randomGen = null)
    {
        if (item.spawnPoint == null)
        {
            Debug.LogError("Item " + item.data.id + " has no spawn point. Unable to spawn item " + itemSpawner.name + " properly, defaulting to transform position");
            item.spawnPoint = item.transform;
        }
        item.transform.MoveAlign(item.spawnPoint, itemSpawner.transform);
        Vector3 vector3 = randomGen == null ? Random.insideUnitSphere : new Vector3((float) randomGen.NextDouble(), (float) randomGen.NextDouble(), (float) randomGen.NextDouble()).normalized * (float) randomGen.NextDouble();
        item.transform.position += Vector3.ProjectOnPlane(vector3 * itemSpawner.randomRadius, itemSpawner.transform.up);
        item.transform.position += Vector3.up * 0.065f;
